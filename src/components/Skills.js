...
export default function Stack() {
  ...
  useEffect(() => {
    fetch("/api/content").then((r) => r.ok && r.json()).then((data) => {
      if (!data?.skills) return;
      setCategories(data.skills.map((saved) => {
        const existing = skillCategories.find((category) => category.id === saved.id) || { icon: Code2, skills: [] };
        return { ...existing, ...saved, skills: saved.skills.map((skill) => ({ ...skill, ...(existing.skills.find((old) => old.name === skill.name) || { icon: Code2, color: "#06B6D4" }) })) };
      }));
    }).catch(() => {});
  }, []);
